...
const Login = () => {
  const [state, stateSet] = useState({ user: "" });
  ...
  const onLogin = async () => {
    if (state.user.length > 0) {
      try {
        ...
        if (!response.errors) {
          state.id = response.data.createUser.id;
          //  state.user = response.data.createUser.name;
          navigate(`/chat`, { state: { user: state.user, userId: state.id } });
        } else {
